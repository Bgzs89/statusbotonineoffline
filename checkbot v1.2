...
local lastStatusUpdate = 0  -- kapan terakhir update status
...
function sendOrEditStatus(updateStatus)
    ...
    -- Hanya update lastUpdateTime kalau status nya di refresh
...
while true do
    ...
    if now - lastStatusUpdate >= 120 then
        -- Tiap 120 detik, update status + reset timer
        sendOrEditStatus(true)
    else
        -- Tiap 30 detik, cuma refresh tulisan "X detik yang lalu"
